...
def _svc():
    ...
    return NutritionService(SqliteNutritionRepository(path), SyntheticFoodProvider())

def test_profile_crud_and_provenance():
    s = _svc()
    s.upsert_profile("person-syn-1", {"context":"GENERAL","targets":{"protein_g":60},"target_source":"USER_CONFIGURED"})
    p = s.get_profile("person-syn-1")
...
def test_planned_and_actual_distinct():
    s = _svc()
    pl = s.record_planned("p1","2026-09-14",[{"food_id":"syn-yogurt","grams":100}])
    ac = s.record_actual("p1","2026-09-14",[{"food_id":"syn-pasta-cooked","grams":100}])
    assert pl["kind"] == "PLANNED" and ac["kind"] == "ACTUAL"
    # daily actual reflects ONLY actual, not planned
    di = s.daily_intake("p1","2026-09-14")
    assert di.get("carb_g") == "25"   # pasta only

def test_ate_as_planned_copies_without_reentry():
    s = _svc()
    pl = s.record_planned("p2","2026-09-14",[{"food_id":"syn-chicken-breast","grams":100}])
    ac = s.ate_as_planned("p2","2026-09-14", pl["id"])
    assert ac["kind"] == "ACTUAL" and ac["provenance"] == "USER_CONFIRMED"
    assert s.daily_intake("p2","2026-09-14")["protein_g"] == "31"
...
def test_daily_gap():
    s = _svc()
    s.upsert_profile("p3", {"targets":{"protein_g":60}})
    s.record_actual("p3","2026-09-14",[{"food_id":"syn-chicken-breast","grams":100}])  # 31
    g = s.daily_gap("p3","2026-09-14")
